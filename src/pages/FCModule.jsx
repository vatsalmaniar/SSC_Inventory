...
export default function FCModule() {
  ...
  const counts = {
    ...
    action:            timelineBatches.filter(b => actionStatuses.includes(effStatus(b))).length,
    waiting:           timelineBatches.filter(b => waitStatuses.includes(effStatus(b))).length,
    all:               timelineBatches.filter(b => actionStatuses.includes(effStatus(b)) || waitStatuses.includes(effStatus(b))).length,
    delivery_created:  timelineBatches.filter(b => effStatus(b) === 'delivery_created').length,
    picking:           timelineBatches.filter(b => effStatus(b) === 'picking').length,
    packing:           timelineBatches.filter(b => effStatus(b) === 'packing').length,
    invoice_generated: timelineBatches.filter(b => effStatus(b) === 'invoice_generated').length,
    eway_generated:    timelineBatches.filter(b => effStatus(b) === 'eway_generated').length,
    dispatched_fc:     timelineBatches.filter(b => effStatus(b) === 'dispatched_fc').length,
  }
    ...
    <Layout pageTitle="Fulfilment Center" pageKey="fc">
      <div className="orders-app">
        ...
        <div className="ph-bento o-bento-flat">
          <Stat label="Action Required" value={counts.action} foot="needs FC action"
            onClick={() => setFilter('action')} />
          <Stat label="Total Active" value={totalActive} foot="in pipeline"
            onClick={() => setFilter('all')} />
          <Stat label="Delivered" value={counts.dispatched_fc} foot={`dispatched ${FY_LABEL}`}
            onClick={() => setFilter('dispatched_fc')} />
          <Stat label="With Accounts" value={counts.waiting} warn={counts.waiting > 0}
            foot="with billing team" onClick={() => setFilter('waiting')} />
          <Stat label="This Filter" value={filtered.length} foot="matching batches" />
        </div>
        ...
        <div className="o-toolbar">
          <div className="o-search">
            ...
            <input placeholder="Search DC, order, customer…" value={search} onChange={e => { setSearch(e.target.value); setPage(1) }}/>
            {search && (
        ...
        {loading ? (
          ...
          <div className="ol-wrap">
            <div className="ol-row ol-head fcl-row">
              <div>DC #</div>
              <div>Customer</div>
              <div>Centre</div>
              <div>Date</div>
            ...
            {filtered.length === 0 ? (
              ...
              <div className="ol-table">
                {paginated.map(b => {
                  ...
                  return (
                    <div key={b.id} className="ol-row ol-data fcl-row" onClick={() => navigate('/fc/' + b.order_id, { state: { dispatch_id: b.id } })}>
