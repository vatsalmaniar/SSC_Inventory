...
export default function PurchaseOrderDetail() {
  ...
  async function init() {
    ...
    const { data: users } = await sb.from('active_people').select('id,name,username,role').neq('role','staff')
    setAllUsers(users || [])
    await loadPO()
  }
...
We request that you check all the details and contact our Procurement team — or ${userName}, who placed this PO — if you have any queries. Replying to this email reaches our Procurement team directly.`
  ...
  // renderPoPdfBlob now lives in src/lib/htmlToPdf.js — shared with the
  // WhatsApp statement sender. Behaviour is unchanged.
  const renderPoPdfBlob = (html, poNumber) =>
    htmlToPdfBlob(html, `${poNumber.split('/')[1] || poNumber}.pdf`)
