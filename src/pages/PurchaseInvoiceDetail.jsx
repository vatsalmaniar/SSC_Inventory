...
export default function PurchaseInvoiceDetail() {
  ...
  async function init() {
    ...
    if (!['accounts','ops','admin','management'].includes(profile?.role)) { navigate('/dashboard'); return }
  ...
  async function loadInvoice(silent) {
    ...
    if (poId) {
      const { data: poData } = await sb.from('purchase_orders').select('id,po_number,vendor_name,status,created_at,total_amount').eq('id', poId).single()
      setPo(poData || null)
    }
