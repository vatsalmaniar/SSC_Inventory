...
import { isWeekOff, istYmd, loadWeekOffOverrides } from '../lib/attendance'
...
export default function PeopleLeave() {
  ...
  async function load(myId, r) {
    ...
    // team view: admin/management + the HR approver
    if (['admin','management'].includes(r) || (myId && hrEmp === myId)) loadTeam(r)
  }
  ...
  return (
    <Layout pageKey="people" pageTitle="Leave">
      <div className="people-app">
            ...
            <div className="acard" style={{marginBottom:14,padding:'18px 20px',display:'flex',alignItems:'flex-start',gap:16,flexWrap:'wrap'}}>
              ...
              <div style={{flex:1,minWidth:220}}>
                <div className="lv-tile-head" style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:8}}>
                  <div style={{fontSize:10,fontWeight:600,letterSpacing:'0.04em',textTransform:'uppercase',color:'var(--muted)'}}>{viewingOther ? `${viewed.emp.full_name} · ` : ''}Leave balance · FY {currentFyLabel()}</div>
                ...
                <div style={{fontSize:30,fontWeight:600,letterSpacing:'-0.025em',lineHeight:1,marginTop:6,fontFamily:"'Geist Mono',monospace"}}>{viewBalNum ?? '—'}<small style={{fontSize:14,color:'var(--muted-2)',fontWeight:500,fontFamily:"'Geist',sans-serif"}}> / {credited} left</small></div>
        ...
        {/* approvals inbox (mine — hidden while viewing someone else) */}
          ...
          <div className="att-card" style={{marginTop:14}}>
            ...
            <div style={{overflowX:'auto'}}>
              <div style={{minWidth:560}}>
                <div style={{display:'grid',gridTemplateColumns:'minmax(160px,1.4fr) 70px 70px 84px 70px 56px',gap:10,padding:'7px 0 5px',fontSize:10,fontWeight:600,letterSpacing:'0.05em',textTransform:'uppercase',color:'var(--muted)',borderBottom:'1px solid var(--line-2)'}}>
      ...
      {show && (
        <Drawer title="Apply Leave" sub={`Balance: ${balNum ?? '—'} / 25`} onClose={()=>setShow(false)}
          ...
          {sandwichPrev.length > 0 && (
            <div style={{fontSize:12,color:'#B45309',background:'#FCF1E4',borderRadius:8,padding:'8px 12px',lineHeight:1.5}}>
