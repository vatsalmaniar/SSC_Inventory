...
export default function PunchButton() {
  ...
  return (
    <>
      <button onClick={openPunch} aria-label={`Check ${isOut ? 'out' : 'in'}`}
        title={isOut ? 'Check out — you are currently checked in' : 'Check in'}
        className={`ly-punch-btn${isOut ? ' is-out' : ''}`}>
        <span className="ly-punch-dot" />
        <span className="ly-punch-label">Check {isOut ? 'Out' : 'In'}</span>
      </button>
