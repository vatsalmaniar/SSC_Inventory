...
has('e-generated note',  "SSC's signature is not required")
has('candidate must sign', 'Please sign and return a copy to accept')
has('basic monthly',     '15,300')
has('basic annual',      '1,83,600')
has('monthly gross row', 'Monthly Gross')
has('deductions column', 'Total Deductions')
has('calculator layout', 'class="calc-cols"')
...
ihas('internship badge',   '<div class="doc-type-badge">Internship</div>')
ihas('internship title',   'Internship<br/>Offer')
ihas('stipend printed',    '12,000')
ihas('duration printed',   '6 months')
ihas('total for the term', '72,000')            // 12,000 x 6
ihas('stipend annexure',   'Stipend<br/>Details')
ihas('term column',        '>Term<')
// Every head must PRINT for an intern, showing zero — not be omitted.
ihas('intern shows PF',       'PF (Employee)')
ihas('intern shows gratuity', 'Gratuity')
ihas('intern shows bonus',    'Bonus')
ihas('intern shows TDS',      'TDS')
ihas('intern stipend row',    'Monthly Stipend')
eq('intern heads are zero',
   (internHtml.match(/<b class="mono">0<\/b>/g) || []).length >= 8, true)
// The things an intern must NOT be promised.
ihas('no salary annexure', 'Compensation<br/>Structure', false)
...
eq('salaried letter unaffected', html.includes('Internship') || html.includes('Stipend'), false)

// Challan-style chrome, and the annexure on its own sheet.
console.log('\nchallan layout')
has('doc-type badge',      '<div class="doc-type-badge">Employment</div>')
has('compensation page',   'Compensation<br/>Structure')
has('two sheets',          'page-break-after')
eq('exactly 2 sheets',     (html.match(/class="sheet"/g) || []).length, 2)
has('A4 print rule',       '@page{size:A4')
has('reference table',     'Offer No.')
has('letterhead repeated', 'SSC Control Pvt. Ltd.')
eq('letterhead on both pages', (html.match(/class="co-name"/g) || []).length, 2)
has('candidate signs',     'Accepted — signature')
